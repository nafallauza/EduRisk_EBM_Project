...
OUTPUT_PATH = BASE_DIR / "output" / "hasil_word_tokenizer.txt"
...
itos = {idx: word for idx, word in enumerate(unique_words)}
...
def decode(token_ids):
    return " ".join([itos.get(idx, "<UNK>") for idx in token_ids])

ids = [stoi[word] for word in words]
...
log_lines = []
log_lines.append("EKSPERIMEN WORD-LEVEL TOKENIZATION")
log_lines.append("=" * 50)
log_lines.append(f"Jumlah kata corpus        : {len(words)}")
...
print("Mulai training Word-Level Tokenization...")
...
generated_ids = model.generate(context, max_new_tokens=90)[0].tolist()
...
log_lines.append(
    "Word-Level Tokenization menghasilkan token berupa kata utuh. "
    "Pendekatan ini membuat hasil teks relatif mudah terbaca karena model tidak perlu menyusun kata dari karakter. "
    "Namun, ukuran vocabulary cenderung lebih besar karena setiap kata unik dianggap sebagai token berbeda."
)
